...
function urgencia(dias: number) {
  if (dias > 60) return { label: "Crítico" };
  if (dias > 45) return { label: "Alto riesgo" };
  return { label: "En riesgo" };
}
          ...
          <section className="flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
            <div className="flex items-center justify-between border-b border-gray-50 px-6 py-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-gray-800" style={{ fontFamily: "var(--font-syne)" }}>Clientes en riesgo de abandono</h2>
              </div>
              <span className="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-700">+35 días sin compra</span>
            </div>
            {enRiesgo.length === 0 ? (
              ...
              <div className="divide-y divide-gray-50">
                {enRiesgo.map((c) => {
                  ...
                  return (
                    <div key={c.id} className={`flex items-center justify-between gap-3 border-l-4 px-5 py-4 ${borderColor} ${rowBg}`}>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-0.5">
                          <p className="text-sm font-semibold text-gray-900 truncate">{c.nombre}</p>
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${badgeColor}`}>{u.label}</span>
                        </div>
                        <p className="text-xs text-gray-500">{c.contacto ?? "—"} · Ticket prom. {fmtK(c.ticket_promedio ?? 0)}</p>
                      </div>
                      <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                        <span className={`text-lg font-bold ${textColor}`} style={{ fontFamily: "var(--font-syne)" }}>{dias}d</span>
                        <div className="flex gap-1">
                          <button title="Llamar" className="flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 transition-colors"><PhoneCall className="h-3.5 w-3.5" /></button>
                          <button title="WhatsApp" className="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-colors"><MessageCircle className="h-3.5 w-3.5" /></button>
                        </div>
                      </div>
                    </div>
                  );
          ...
          <section className="flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
            <div className="flex items-center justify-between border-b border-gray-50 px-6 py-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-gray-800" style={{ fontFamily: "var(--font-syne)" }}>Concentración de ingresos</h2>
              </div>
              <span className="text-xs text-gray-400">Pareto · {pLabel}</span>
            </div>
            <div className="flex-1 px-6 py-5 flex flex-col gap-5">
              <div className={`flex items-start gap-3 rounded-xl border p-4 ${riesgo === "alto" ? "bg-rose-50 border-rose-100" : riesgo === "medio" ? "bg-amber-50 border-amber-100" : "bg-emerald-50 border-emerald-100"}`}>
                <ShieldAlert className={`h-5 w-5 flex-shrink-0 mt-0.5 ${riesgo === "alto" ? "text-rose-500" : riesgo === "medio" ? "text-amber-500" : "text-emerald-500"}`} />
                <div>
                  <p className={`text-sm font-semibold ${riesgo === "alto" ? "text-rose-700" : riesgo === "medio" ? "text-amber-700" : "text-emerald-700"}`}>
                    {riesgo === "alto" ? `⚠️ Riesgo alto — top 3 representan el ${top3pct.toFixed(0)}%` : riesgo === "medio" ? `Riesgo moderado — ${top3pct.toFixed(0)}% en 3 clientes` : `Cartera bien diversificada — top 3 representa ${top3pct.toFixed(0)}%`}
                  </p>
                  <p className={`text-xs mt-0.5 ${riesgo === "alto" ? "text-rose-500" : riesgo === "medio" ? "text-amber-500" : "text-emerald-600"}`}>
                    {riesgo === "alto" ? "Perder un cliente clave golpearía fuertemente el negocio." : "Sigue diversificando tu base de clientes."}
                  </p>
                </div>
              </div>
              {pareto.length === 0 ? <p className="text-center text-sm text-gray-300 py-8">Sin datos en este periodo.</p> : (
                <div className="space-y-4">
                  {pareto.map((c) => {
                    const i = c.rank - 1;
                    return (
                      <div key={c.rank} className="flex items-center gap-4">
                        <span className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold text-white ${BAR[i] ?? "bg-gray-400"}`}>{c.rank}</span>
              ...
              {pareto.length > 0 && (
                <div className="mt-auto rounded-xl bg-gray-50 px-4 py-3">
                  <p className="text-xs text-gray-500">Top 3 → <strong className={riesgo === "alto" ? "text-rose-600" : "text-indigo-600"}>{top3pct.toFixed(1)}%</strong> del total · {pLabel}</p>
                </div>
              )}
            </div>
          </section>
